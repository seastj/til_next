...
- `/src/app 폴더` :
  ...
  - `App Router 버전`으로 진행시 `app 폴더`가 존재함.
...
- search 폴더 이동함
- /app/page.tsx 이동함
- /app/page.module.css 이동함

## 4. React 19 - Server Component 의 이해

- 클라이언트 즉, 웹브라우저에서 JS 번들로 실행되는 컴포넌트를 Client Component 라고 한다.
- 서버 즉, PC 에서 미리 생성해 주는 컴포넌트를 Server Component 라고 한다.

### 4.1. 구분

- 기존의 React 는 html 을 미리 생성하지 않는다. (웹브라우저가 HTML 생성함)
- Next는 Server 에서 미리 html 을 만든다. (만들어진 HTML 출력시킨 후 다시 JS 번들을 전달함 > 기능 작동함(Hydration))
- Next는 프리렌더링 단계로 HTML 즉 내용을 미리 만들어서 보여주기 때문에 FCP 가 빠르다.
...
- 실시간 html 을 생성하는 형태임.
- 컴포넌트의 상단에 `"use client"` 라는 글자를 적어주어야 작동됨.

### 4.5. 컴포넌트를 생성하는 기준(간략한 기준)

- Next 는 Server Component 가 기준이다.
- 만약, `use 훅`들을 사용해야 하는 경우는 "use client" 명시 해야한다.
- 만약, `이벤트 핸들러` 들을 사용해야 하는 경우 "use client" 명시 해야한다.
- 만약, 웹 브라우저 에서만 실행되는 라이브러리 라면 "use client" 명시 해야한다.
- 원칙은 Client Component 에 Server Component 를 import 하지 않도록 하자.
- Client Component 에 Server Component 를 import 하면 Client Component 가 됨.
...
- import 로 Client Component 로 활용 (권장하지 않는 방식)
...
- `/src/component/SearchBar.module.css`
...
- `/src/app/(with-search)/search/page.module.css` 업데이트
