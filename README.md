...
### 3.3. Next 의 `Server Action` 으로 수정을 한다면
...
function ReviewForm() {
  ...
  async function createReviewAction(formData: FormData) {
    ...
    // FormData { content: 'aaa', author: 'bbb' }
    // 혹시 데이터가 없으면 null 인데 아래처럼 처리가 옳은가?
    // const content = formData.get("content") as string;
    // const author = formData.get("author") as string;
    // 아래가 null 도 담을 수 있으므로 as 보다는 좋다.
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLICE_API_URL}/products`,
        {
          method: "POST",
...
- `/src/actions 폴더` 생성
- `/src/actions/create-review-action.ts` 생성
...
export async function createReviewAction(formData: FormData) {
  ...
  try {
    ...
    const { id } = await response.json();
    console.log("상품 리뷰 등록 결과 id : ", id);
    // 리뷰가 등록되었으면 리렌더링으로 출력을 시켜야함.
    // - 서버에서 호출해야 한다.
    // - 지금까지 가지고 있던 cache 를 무효화해야 한다.
...
- 어려움. 그러나 필수
- Server Action 은 자료를 등록하는 form 태그에 action 이다
- 연속으로 등록을 방지해야 한다
...
// Action 용 함수
// useActionsState 활용시 : Action 의 상태도 전달을 하는 형태로 적용필요
...
- `/src/actions/DeleteBt.tsx 파일` 생성
...
const DeleteBt = ({ id }: GoodDataType) => {
...
- `/src/actions/DeleteBt.module.css 파일` 생성
